...
namespace SS2
{
	...
	public class RTDebugUnit : MonoBehaviour
	{
		...
		public string[] DumpSatellites()
		{
			...
			var i = 0;
			foreach (var satellite in SS2Core.Instance.Network)
			{
		...
		public string[] DumpAntennas()
		{
			...
			var i = 0;
			foreach (var antenna in SS2Core.Instance.Antennas)
			{
				data.Add($"    {i++}: {antenna}");
			}
		...
		public string[] DumpEdges()
		{
			...
			var i = 0;
			foreach (var edge in SS2Core.Instance.Network.Graph)
			{
				data.Add($"    {i++}: {edge.Key}");
				var j = 0;
		...
		public string[] DumpConnectionTests()
		{
			...
			var i = 0;
			foreach (var sat1 in SS2Core.Instance.Network)
			{
				var j = 0;
				foreach (var sat2 in SS2Core.Instance.Network)
				{
