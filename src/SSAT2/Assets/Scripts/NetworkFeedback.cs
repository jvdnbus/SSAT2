...
namespace SS2
{
	public class NetworkFeedback
	{
		...
		{
			if (SS2Core.Instance != null && SS2Core.Instance.Network != null &&
				target != Guid.Empty)
			{
				ISatellite targetSat = SS2Core.Instance.Network[target];
				if (targetSat != null)
				{
				...
				try
				{
					CelestialBody targetPlanet = SS2Core.Instance.Network.Planets[target];
		...
		public static int countInCone(IAntenna antenna, Guid target)
		{
			...
			try
			{
				Vector3d myPos = SS2Core.Instance.Network[antenna.Guid].Position;
				Vector3d targetDir = (targetPosition(target) - myPos).normalized;
				CelestialBody myBody = targetBody(target);

				int count = 0;
				foreach (ISatellite sat in SS2Core.Instance.Satellites)
				{
		...
		public static KeyValuePair<string, UnityEngine.Color> tryConnection(IAntenna antenna, Guid target)
		{
					...
					target != Guid.Empty && target != NetworkManager.ActiveVesselGuid)
			{
				...
				ISatellite mySat = SS2Core.Instance.Network[antenna.Guid];
				if (mySat == null)
