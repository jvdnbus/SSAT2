...
namespace SS2
{
	...
	public class NetworkRenderer : MonoBehaviour
	{
		...
		{
			var renderer = Shortcuts.MapViewManager.MapViewCamera.gameObject.GetComponent<NetworkRenderer>();
			if (renderer)
			{
				Destroy(renderer);
			}

			renderer = Shortcuts.MapViewManager.MapViewCamera.gameObject.AddComponent<NetworkRenderer>();
			SS2Core.Instance.Network.OnLinkAdd += renderer.OnLinkAdd;
			SS2Core.Instance.Network.OnLinkRemove += renderer.OnLinkRemove;
			SS2Core.Instance.Satellites.OnUnregister += renderer.OnSatelliteUnregister;
		...
		public void OnGUI()
		{
			...
			{
				foreach (ISatellite s in SS2Core.Instance.Satellites.FindCommandStations().Concat(SS2Core.Instance.Network.GroundStations.Values))
				{
		...
		{
			List<IAntenna> antennas = (ShowCone ? SS2Core.Instance.Antennas.Where(
										ant => ant.Powered && ant.CanTarget && SS2Core.Instance.Satellites[ant.Guid] != null
			...
			{
				var center = SS2Core.Instance.Network.GetPositionFromGuid(antennas[i].Target);
		...
		private bool CheckVisibility(BidirectionalEdge<ISatellite> edge)
		{
			...
			var satellite = SS2Core.Instance.Satellites[vessel];
			if (satellite != null && ShowPath)
			{
				var connections = SS2Core.Instance.Network[satellite];
			...
			{
				var satellites = SS2Core.Instance.Network.ToArray();
				for (int i = 0; i < satellites.Length; i++)
				{
					var connections = SS2Core.Instance.Network[satellites[i]]; // get the working-connection path of every satellite
					if (connections.Any() && connections[0].Contains(edge))
						return true;
				}
			}
		...
		private Color CheckColor(BidirectionalEdge<ISatellite> edge)
		{
			...
			{
				var satA = SS2Core.Instance.Satellites[edge.A.Guid];
				var satB = SS2Core.Instance.Satellites[edge.B.Guid];
		...
		{
			SS2Core.Instance.Network.OnLinkAdd -= OnLinkAdd;
			SS2Core.Instance.Network.OnLinkRemove -= OnLinkRemove;
			SS2Core.Instance.Satellites.OnUnregister -= OnSatelliteUnregister;
		}
	}
}