using SS2.RangeModel;
using SS2.Modules;
using SS2.SimpleTypes;
using System;
using System.Collections.Generic;
using System.Linq;
using WrappedEvent = SS2.FlightComputer.UIPartActionMenuPatcher.WrappedEvent;
...
namespace SS2.API
{
	public static class API
	{
		...
		{
			if (SS2Core.Instance != null) return true;
			return false;
		}
		...
		public static void EnableInSPC(bool state)  // its advised that modders who need RTCore active in the SPC should set this from the MainMenu Scene
		{
			...
			if (!enabledInSPC && SS2Core.Instance != null && HighLogic.LoadedScene == GameScenes.SPACECENTER)
			{
				RTLog.Verbose("RemoteTech is terminated in Space Center scene", RTLogLevel.API);
				SS2Core.Instance.OnDestroy();
			}
		}
		...
		{
			if (SS2Core.Instance == null) return false;
			var satellite = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();
		...
		{
			if (SS2Core.Instance == null) return;
			var satellite = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();
		...
		public static bool HasAnyConnection(Guid id)
		{
			...
			var hasConnection = SS2Core.Instance.Network[satellite].Any();
		...
		public static bool HasConnectionToKSC(Guid id)
		{
			...
			var connectedToKerbin = SS2Core.Instance.Network[satellite].Any(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid));
		...
		public static bool HasDirectGroundStation(Guid id)
		{
			...
			var targetsGroundStation = SS2Core.Instance.Network[satellite].Any(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Links.FirstOrDefault().Target.Guid));
		...
		{
			if (SS2Core.Instance == null) return null;
			var satellite = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();
			if (satellite == null) return null;

			var namedGroundStation = SS2Core.Instance.Network[satellite].Where
				(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Links.FirstOrDefault().Target.Guid)).Min().Goal.Name;
		...
		public static string GetFirstHopToKSC(Guid id)
		{
			...
			var namedSatellite = SS2Core.Instance.Network[satellite].Where
				(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid)).Min().Links.FirstOrDefault().Target.Name;
		...
		{
			if (SS2Core.Instance == null) return new Guid[] { };

			var satellite = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();
			if (satellite == null) return new Guid[] { };

			if (!SS2Core.Instance.Network[satellite].Any(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid))) return new Guid[] { };

			List<NetworkLink<ISatellite>> bestRouter = SS2Core.Instance.Network[satellite].Where(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid)).Min().Links;
		...
		{
			if (SS2Core.Instance == null) return null;
			var satellite = SS2Core.Instance.Network.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();
			if (satellite == null) return null;
		...
		{
			if (SS2Core.Instance == null) return double.PositiveInfinity;
			var satellite = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(id)).FirstOrDefault();

			if (satellite == null) return double.PositiveInfinity;

			if (!SS2Core.Instance.Network[satellite].Any()) return double.PositiveInfinity;
			var shortestDelay = SS2Core.Instance.Network[satellite].Min().Delay;
		...
		public static double GetSignalDelayToKSC(Guid id)
		{
			...
			if (!SS2Core.Instance.Network[satellite].Any(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid))) return double.PositiveInfinity;
			var signalDelaytoKerbin = SS2Core.Instance.Network[satellite].Where(r => SS2Core.Instance.Network.GroundStations.ContainsKey(r.Goal.Guid)).Min().Delay;
		...
		{
			if (SS2Core.Instance == null) return double.PositiveInfinity;
			var satelliteA = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(a)).FirstOrDefault();
			var satelliteB = SS2Core.Instance.Satellites.Where(sat => sat.Guid.Equals(b)).FirstOrDefault();

			if (satelliteA == null || satelliteB == null) return double.PositiveInfinity;

			Func<ISatellite, IEnumerable<NetworkLink<ISatellite>>> neighbors = SS2Core.Instance.Network.FindNeighbors;
		...
		public static bool QueueCommandToFlightComputer(ConfigNode externalData)
		{
			...
			try
			{
				...
				FlightComputer.FlightComputer computer = SS2Core.Instance.Satellites[externalVesselId].FlightComputer;
		...
		{
			if (SS2Core.Instance == null) return 0.0;

			//sanity check
			var satelliteA = SS2Core.Instance.Network.Where(sat => sat.Guid.Equals(sat_a)).FirstOrDefault();
			var satelliteB = SS2Core.Instance.Network.Where(sat => sat.Guid.Equals(sat_b)).FirstOrDefault();
