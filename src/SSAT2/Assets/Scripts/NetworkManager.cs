...
namespace SS2
{
	...
	public partial class NetworkManager : IEnumerable<ISatellite>
	{
		...
		public int Count { get { return SS2Core.Instance.Satellites.Count + GroundStations.Count; } }
		...
		public ISatellite this[Guid guid]
		{
			get
			{
				...
				ISatellite vesselSatellite = SS2Core.Instance.Satellites[guid];
				ISatellite activeSatellite = (guid == ActiveVesselGuid ? SS2Core.Instance.Satellites[activeVessel] : null);
		...
		public NetworkManager()
		{
			...
			SS2Core.Instance.Satellites.OnRegister += OnSatelliteRegister;
			SS2Core.Instance.Satellites.OnUnregister += OnSatelliteUnregister;
			SS2Core.Instance.OnPhysicsUpdate += OnPhysicsUpdate;
		}

		public void Dispose()
		{
			if (SS2Core.Instance != null)
			{
				SS2Core.Instance.OnPhysicsUpdate -= OnPhysicsUpdate;
				SS2Core.Instance.Satellites.OnRegister -= OnSatelliteRegister;
				SS2Core.Instance.Satellites.OnUnregister -= OnSatelliteUnregister;
		...
		{
			var count = SS2Core.Instance.Satellites.Count;
			if (count == 0) return;
			int baseline = (count / REFRESH_TICKS);
			int takeCount = baseline + (((mTick++ % REFRESH_TICKS) < (count - baseline * REFRESH_TICKS)) ? 1 : 0);
			IEnumerable<ISatellite> commandStations = SS2Core.Instance.Satellites.FindCommandStations();
			foreach (VesselSatellite s in SS2Core.Instance.Satellites.Concat(SS2Core.Instance.Satellites).Skip(mTickIndex).Take(takeCount))
			{
				UpdateGraph(s);
			...
			mTickIndex = mTickIndex % SS2Core.Instance.Satellites.Count;
		}
		...
		{
			return SS2Core.Instance.Satellites.Cast<ISatellite>().Concat(GroundStations.Values).GetEnumerator();
		}
